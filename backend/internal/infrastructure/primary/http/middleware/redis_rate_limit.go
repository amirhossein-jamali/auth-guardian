// internal/infrastructure/primary/http/middleware/redis_rate_limit.go
package middleware
...
// Handle provides the Gin middleware function for rate limiting
func (m *RedisRateLimiterMiddleware) Handle() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			m.logger.Error("Rate limiting error",
				model.NewField("ip", key),
		...
		if !allowed {
			m.logger.Warn("Rate limit exceeded",
				model.NewField("ip", key),
				model.NewField("rate", m.rate),
				model.NewField("window", m.window.String()))
