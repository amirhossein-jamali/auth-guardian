...
func (l *GormLoggerAdapter) Info(ctx context.Context, msg string, data ...interface{}) {
	if l.logLevel >= gormlogger.Info {
		l.logger.Info(fmt.Sprintf(msg, data...), model.NewField("source", "gorm"))
	}
}
...
func (l *GormLoggerAdapter) Warn(ctx context.Context, msg string, data ...interface{}) {
	if l.logLevel >= gormlogger.Warn {
		l.logger.Warn(fmt.Sprintf(msg, data...), model.NewField("source", "gorm"))
	}
}
...
func (l *GormLoggerAdapter) Error(ctx context.Context, msg string, data ...interface{}) {
	if l.logLevel >= gormlogger.Error {
		l.logger.Error(fmt.Sprintf(msg, data...), model.NewField("source", "gorm"))
	}
}
...
func (l *GormLoggerAdapter) Trace(ctx context.Context, begin time.Time, fc func() (string, int64), err error) {
	...
	fields := []model.Field{
		model.NewField("source", "gorm"),
		model.NewField("elapsed", elapsed.String()),
		model.NewField("rows", rows),
		model.NewField("sql", sql),
	}

	// Add error field if there's an error
	if err != nil {
		fields = append(fields, model.NewField("error", err.Error()))
	}
